...
This model builds on top of Uri Wilensky's Preferential Atachment model on the netlogo
library and it was built to give intuition on the following question on Brilliant.org
...
Assume that the network has grown large enough so that its statistical properties are effectively constant. The fractions of nodes that have connections to exactly 7 other nodes is given by some fraction a/b, where a and b are positive coprime integers. What is the value of a + b?

## HOW IT WORKS

The model starts with two nodes connected by an edge.

At each step, a new node is added.  A new node picks an existing node to connect to randomly.
